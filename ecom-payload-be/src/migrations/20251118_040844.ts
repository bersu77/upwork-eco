...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE TABLE \`facets\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`name\` text NOT NULL,
  	\`code\` text NOT NULL,
  	\`generate_slug\` integer DEFAULT true,
  ...
  `)
  await db.run(sql`CREATE UNIQUE INDEX \`facets_code_idx\` ON \`facets\` (\`code\`);`)
  await db.run(sql`CREATE UNIQUE INDEX \`facets_slug_idx\` ON \`facets\` (\`slug\`);`)
  await db.run(sql`CREATE INDEX \`facets_updated_at_idx\` ON \`facets\` (\`updated_at\`);`)
  await db.run(sql`CREATE INDEX \`facets_created_at_idx\` ON \`facets\` (\`created_at\`);`)
  await db.run(sql`CREATE TABLE \`facet_values\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`name\` text NOT NULL,
  	\`code\` text NOT NULL,
  	...
  	\`updated_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL,
  	\`created_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL,
  	FOREIGN KEY (\`facet_id\`) REFERENCES \`facets\`(\`id\`) ON UPDATE no action ON DELETE set null
  );
  `)
  await db.run(sql`CREATE INDEX \`facet_values_facet_idx\` ON \`facet_values\` (\`facet_id\`);`)
  await db.run(sql`CREATE UNIQUE INDEX \`facet_values_slug_idx\` ON \`facet_values\` (\`slug\`);`)
  await db.run(sql`CREATE INDEX \`facet_values_updated_at_idx\` ON \`facet_values\` (\`updated_at\`);`)
  await db.run(sql`CREATE INDEX \`facet_values_created_at_idx\` ON \`facet_values\` (\`created_at\`);`)
  ...
  `)
  await db.run(sql`CREATE UNIQUE INDEX \`shipping_methods_code_idx\` ON \`shipping_methods\` (\`code\`);`)
  await db.run(sql`CREATE UNIQUE INDEX \`shipping_methods_slug_idx\` ON \`shipping_methods\` (\`slug\`);`)
  ...
  await db.run(sql`CREATE TABLE \`addresses\` (
  	...
  	\`province\` text,
  	\`phone_number\` text,
  	\`default_shipping_address\` integer DEFAULT false,
  	\`default_billing_address\` integer DEFAULT false,
  	\`updated_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL,
  	\`created_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL,
  ...
  await db.run(sql`CREATE TABLE \`products_rels\` (
  	...
  	FOREIGN KEY (\`products_id\`) REFERENCES \`products\`(\`id\`) ON UPDATE no action ON DELETE cascade,
  	FOREIGN KEY (\`categories_id\`) REFERENCES \`categories\`(\`id\`) ON UPDATE no action ON DELETE cascade,
  	FOREIGN KEY (\`facet_values_id\`) REFERENCES \`facet_values\`(\`id\`) ON UPDATE no action ON DELETE cascade
  );
  `)
  ...
  await db.run(sql`CREATE INDEX \`products_rels_facet_values_id_idx\` ON \`products_rels\` (\`facet_values_id\`);`)
  ...
  await db.run(sql`CREATE INDEX \`_products_v_rels_facet_values_id_idx\` ON \`_products_v_rels\` (\`facet_values_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`carts_payments\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`carts_payments_order_idx\` ON \`carts_payments\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`carts_payments_parent_id_idx\` ON \`carts_payments\` (\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE \`carts_coupon_codes\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`code\` text NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`carts_coupon_codes_order_idx\` ON \`carts_coupon_codes\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`carts_coupon_codes_parent_id_idx\` ON \`carts_coupon_codes\` (\`_parent_id\`);`)
  ...
  await db.run(sql`CREATE INDEX \`carts_shipping_address_idx\` ON \`carts\` (\`shipping_address_id\`);`)
  await db.run(sql`CREATE INDEX \`carts_billing_address_idx\` ON \`carts\` (\`billing_address_id\`);`)
  await db.run(sql`CREATE INDEX \`carts_shipping_method_idx\` ON \`carts\` (\`shipping_method_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`payload_locked_documents_rels\` (
  	...
  	FOREIGN KEY (\`facets_id\`) REFERENCES \`facets\`(\`id\`) ON UPDATE no action ON DELETE cascade,
  	FOREIGN KEY (\`facet_values_id\`) REFERENCES \`facet_values\`(\`id\`) ON UPDATE no action ON DELETE cascade,
  ...
  await db.run(sql`CREATE INDEX \`payload_locked_documents_rels_facets_id_idx\` ON \`payload_locked_documents_rels\` (\`facets_id\`);`)
  await db.run(sql`CREATE INDEX \`payload_locked_documents_rels_facet_values_id_idx\` ON \`payload_locked_documents_rels\` (\`facet_values_id\`);`)
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  await db.run(sql`DROP TABLE \`facets\`;`)
  await db.run(sql`DROP TABLE \`facet_values\`;`)
  await db.run(sql`DROP TABLE \`shipping_methods\`;`)
  ...
  await db.run(sql`DROP TABLE \`carts_payments\`;`)
  await db.run(sql`DROP TABLE \`carts_coupon_codes\`;`)
